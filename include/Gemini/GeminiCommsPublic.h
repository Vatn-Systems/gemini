...
Parameters:
    sonarID             sonar ID
    hardReset           Forces a reset of the HAL - Should never be need to be changed.
                        Only use case is for reloading/resetting the GeminiComms DLL
...
****************************************/
PUB_GEMINI_COMMS int  GEM_StartGeminiNetworkWithResult(unsigned short sonarID, bool hardReset = false);
...
Returns:                None
****************************************/
PUB_GEMINI_COMMS void GEM_SetGeminiSoftwareMode(const char *softwareMode);
...
Returns:                None
****************************************/
PUB_GEMINI_COMMS void GEMX_SetAltSonarIPAddressWithFlag(unsigned short sonarID, unsigned int ipAddress,
                            unsigned int subnetMask, unsigned int broadcastStatus,
                            unsigned int broadcastPing);

/*************************************//**
Purpose: An alternative to GEMX_SetAltSonarIPAddress() for toggling packet broadcast options.
                 Uses the alternate IP address and subnet mask of the Gemini Sonar for communication.
         e.g. IP address : 192.168.2.201 (UInt32 = 3232236233), Subnet : 255.255.255.0 (UInt32 =
                 4294967040).
         The default param value for broadcastStatus and broadcastPing is 0x0. In this state,
         Status packet broadcasts are ENABLED and Ping packet broadcasts are DISABLED.
         To DISABLE the broadcast of Status packets, set the param value to 0x01234567.
         To ENABLE the direct broadcast of Ping Head, Line & Tail packets (720is only), set the param
         value to 0x01234567, which will set packet destination addr. to xxx.xxx.xxx.255.

Parameters:
    sonarID             Sonar ID
    ipAddress           IP Address = 3232236233
    subnetMask			Subnet Mask = 4294967040
    broadcastStatus		Broadcast Status = 0x0 (default ON)
    broadcastPing		Broadcast Ping = 0x0 (default (OFF)
...
****************************************/
PUB_GEMINI_COMMS bool GEMX_SetNoiseReduction(unsigned short sonarID, bool enable);
...
Returns:                None
****************************************/
PUB_GEMINI_COMMS void GEMX_720imEnableH264(unsigned short sonarID, bool enableH264);

/*************************************//**

Purpose: Power down the DA Board
...
PUB_GEMINI_COMMS void GEMX_SetDABoardPowerOff(unsigned short sonarID);
