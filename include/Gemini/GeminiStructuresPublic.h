...
enum FREQUENCY{
    ...
    FREQUENCY_COMBINED  // Delivers interleaved low/high frequency pings
};
 ...
 * \brief 720imRegularComPortConfig
 */
struct ComPortConfig{
    bool            m_negotiate;
    bool            m_rs232;
    ...
    /*
    ** Com port configuration class defined in GeminiStructurePublic.h file
    */
    ComPortConfig()
    : m_negotiate( false )
    , m_rs232( true )
    ...
    ComPortConfig( bool negotiate, bool rs232, unsigned int baudRate, const char* portName )
    : m_negotiate( negotiate )
    , m_rs232( rs232 )
    , m_baudRate( baudRate )
    {
        snprintf( m_portName, sizeof( m_portName ), "%s", portName );
    }

    ComPortConfig( const char* portName)
    : m_negotiate( false )
    , m_rs232( true )
    , m_baudRate( 115200 )
    {
    ...
    ComPortConfig( const ComPortConfig& comPortConfig )
    {
        *this = comPortConfig;
    }
    ComPortConfig& operator = ( const ComPortConfig& comPortConfig )
    {
        m_negotiate = comPortConfig.m_negotiate;
        m_rs232     = comPortConfig.m_rs232;
        m_baudRate  = comPortConfig.m_baudRate;
        snprintf( m_portName, sizeof( m_portName ), "%s", comPortConfig.m_portName );
        return *this;
    }

    bool operator == ( const ComPortConfig& comPortConfig ) const
    {
        return (
            ( m_negotiate == comPortConfig.m_negotiate ) &&
            ( m_rs232     == comPortConfig.m_rs232 ) &&
            ( m_baudRate  == comPortConfig.m_baudRate ) &&
            ( strncmp( m_portName, comPortConfig.m_portName, strlen( m_portName) ) == 0 )
            );
    }

    void Set( bool negotiate, bool rs232, unsigned int baudRate, const char* portName )
    {
        if( strlen( portName ) && strcmp( portName, m_portName ) != 0 )
        {
            m_negotiate = negotiate;
            m_rs232     = rs232;
            m_baudRate  = baudRate;
